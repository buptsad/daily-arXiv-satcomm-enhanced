...
TL;DR: 澳大利亚 ISP 互联网络通过多层模型验证，网络多样性即使面临共享风险也能提供冗余，凸显多元生态的重要性。


<details>
  <summary>Details</summary>
Motivation: 研究澳大利亚独立互联网服务提供商(ISP)网络之间的相互作用，探讨互联点(IXP)的互联对单个网络故障的冗余效果，并评估共享风险链路组(SRLG)导致的集中故障可能带来的脆弱性。

Method: 构建通用的多层网络模型，每一层表示一个ISP网络，另设一层为IXP，负责跨ISP的互联；以六大 ISP 为主体，建立特定的澳大利亚模型；在该模型上进行故障分析，模拟单个链路或 SRLG 故障情况。

Result: 即使存在共享风险链路，网络多样性仍能实现冗余，证明多元网络生态系统的韧性。

Conclusion: 网络多样性与互联点互联能显著提升容错能力，建议在设计中充分考虑 SRLG 等共享风险因素。
...
TL;DR: 本研究设计并公开了首个完整5G系统资产管理壳（AAS），包括5G用户设备和网络AAS，遵循5G-ACIA、Plattform Industrie 4.0及3GPP标准，显著简化了5G网络与工业4.0系统的集成流程。


<details>
  <summary>Details</summary>
Motivation: 在智能制造领域，5G的广泛部署需要统一、可复用的接口来降低网络配置的复杂性，并支持工业4.0应用紧密集成。

Method: 根据5G-ACIA、Plattform Industrie 4.0及3GPP标准，构建并实现了包含5G UE（用户设备）AAS和5G NW（网络）AAS的完整系统架构，并将其公开发布。

Result: 成功实现并公开了首个完整5G系统AAS，提供必要的数据和能力，支持5G与工业4.0系统的快速集成。

Conclusion: 通过设计和发布完整的5G系统资产管理壳（AAS），论文展示了一种减少5G网络管理复杂度并促进5G与工业4.0系统无缝集成的方案。
...
TL;DR: 5G V2N2V+MEC可为关键V2X服务提供低延迟，需本地对等/联合边缘化才能解决非对称网络问题，可补V2V提升可靠性与卸载侧链流量。


<details>
  <summary>Details</summary>
Motivation: 在车辆与网络互联体系中寻找支持关键V2X服务的可行方案，以提升安全与通信效率。

Method: 评估单运维与多运营商场景下不同网络部署的端到端延迟，探讨MEC/本地对等点与共享数据中心/联合边缘计算对系统性能的影响。

Result: MEC部署能够满足低延迟要求，但需本地对等点、共享数据中心或MEC联合化以克服网络非对称性挑战。

Conclusion: 5G V2N2V通信在MEC基础网络部署下能够满足关键的V2X服务需求，且可作为V2V的补充方案实现更高可靠性与流量卸载。
...
### [7] [Conflict-Aware Retriever Editing for Knowledge Injection Attacks on LLM-Based RAG Systems](https://arxiv.org/abs/2606.18310)
...
### [8] [TIGER: Inverting Transformer Gradients via Embedding-Subspace Distance Optimization](https://arxiv.org/abs/2606.18312)
*William Kalikman,Ivo Petrov,Dimitar I. Dimitrov,Martin Vechev*

Main category: cs.CR

TL;DR: TIGER将注意力梯度的子空间信息转为可微目标，直接优化token嵌入，让梯度反演攻击在transformer上更高效、鲁棒，尤其能突破DP防护。


<details>
  <summary>Details</summary>
Motivation: 在联邦学习中，虽然数据保持在本地，但梯度更新可能泄露输入信息。现有针对transformer的梯度反演攻击要么通过代价高昂且不稳定的优化搜索代币，要么利用注意力梯度的低秩特性进行子空间定位并做离散代币匹配，但这些方法在面对数值噪声（如量化、差分隐私）时易碎或扩展性差。

Method: 提出TIGER（Continuous Gradient Inversion Attack），把子空间信号转为可微目标，直接对代币嵌入执行梯度下降，使嵌入距离子空间最小化，避免离散搜索和全梯度匹配。

Result: 实验表明：在仅编码器的transformer模型中，TIGER在重建质量和运行时间上显著优于现有攻击；在解码器模型中，它对噪声更鲁棒，能够在差分隐私防护的联邦学习环境下成功重构输入。

Conclusion: TIGER有效利用了注意力梯度低秩特性，并通过连续优化提升了反演攻击的效率和鲁棒性，为transformer在联邦学习中的安全评估提供了强有力的工具。
...
### [9] [TopVenues: A Reproducible Corpus and Tooling Substrate for Cybersecurity Literature Reviews](https://arxiv.org/abs/2606.18320)
...
### [10] [Agentra: A Supervisable Multi-Agent Framework for Enterprise Intrusion Response](https://arxiv.org/abs/2606.18325)
...
### [11] [SafeClawBench: Separating Semantic, Audit-Evidence, and Sandbox Harm in Tool-Using LLM Agents](https://arxiv.org/abs/2606.18356)
...
### [12] [Evaluating the Effectiveness of LLMs in Aiding Compliance Testing of PKCS#1-v1.5](https://arxiv.org/abs/2606.18405)
*Polina Kozyreva,Endadul Hoque*

Main category: cs.CR

TL;DR: LLM+语法突变可补全传统测试，但幻觉频发，需谨慎应用并结合手工检查。


<details>
  <summary>Details</summary>
Motivation: 传统随机或手工生成的测试输入难以覆盖具有结构与语义约束的协议行为，且现有工具需专业知识，亟需更通用、更自动化的检测方法。

Method: 对TLV编码协议PKCS#1 v1.5进行语法级突变与LLM自动生成测试脚本，并通过实验验证多种实现。

Result: 在48个实现中成功复现13项违规中的10项，包括全部5类伪造错误，并发现1项新缺陷；LLM幻觉发生率82.5%，显著限制效果。

Conclusion: 利用语言模型（LLM）进行代码合成与语法级突变在协议规范合规测试中表现出可行性，但受限于LLM的幻觉问题，导致语义准确性仅为17.5%。
...
### [13] [Understanding the "Airport" Censorship Circumvention Ecosystem in China](https://arxiv.org/abs/2606.18427)
*Rumaisa Habib,Mingshi Wu,Shiva Shahandeh,Min Ni,Eric Wustrow,Zakir Durumeric*

Main category: cs.CR

TL;DR: 在中国，订阅式审查规避工具“航站楼”非常流行，性能优于直连，但支付、政府打击和配置问题成为新风险。


<details>
  <summary>Details</summary>
Motivation: 了解中国地下市场中的订阅式审查规避工具的流行程度、技术特点与风险。

Method: 结合问卷调查、社交媒体分析（Telegram频道抓取）和主动网络测量，系统研究了航站楼生态。

Result: 在1667名受访者中超过一半使用航站楼；识别到3431个活跃航站楼；35个订阅后发现其多跳架构性能往往优于直接连接，但存在支付等安全隐患。

Conclusion: 论文指出，航站楼（censorship circumvention proxies）是中国最受欢迎的现成工具，但其存在支付、政府打击和配置困难等安全与管理挑战，且其多跳架构有时可优于直接连接；这提示未来对抗审查技术需兼顾性能与可靠性。
...
### [14] [Ghost Vectors: Soft-Deleted Embeddings Remain Reconstructible in HNSW Vector Databases](https://arxiv.org/abs/2606.18497)
*Chandranil Chakraborttii,Jackeline García Alvarado,Sitora Abdulofizova,Shivanshu Dwivedi*

Main category: cs.CR

TL;DR: RAG 软删除易导致敏感信息泄露；逆向模型可大幅恢复数据；Epoch Key Rotation 能迅速彻底删除并提供审计。


<details>
  <summary>Details</summary>
Motivation: RAG 系统在数据删除时仅软删除，存留向量引发 GDPR、HIPAA 等监管合规性风险，需要确保删除操作能彻底擦除敏感信息。

Method: ①分析三种 HNSW 实现，通过直接访问存储层原始索引文件实现软删除向量的恢复；②使用 Vec2Text 逆向模型对不同领域数据（文本、生物医学、图像）进行复原实验；③设计 Epoch Key Rotation，将向量加密并在删除时销毁密钥，同时产生 ECDSA 签名的审计记录，以验证删除合法性。

Result: 在 Wikipedia BLP 数据集恢复 25.5% 人名、46.4% 地理位置；NIH Synthea 数据集患者年龄、性别 100% 恢复；软删除图像嵌入实现 100% 组织分类，面部身份 99% 恢复；Epoch Key Rotation 将成功恢复率降至 0%，耗时约 2.5 ms（500 向量，≈0.005 ms/记录）。

Conclusion: 软删除操作未真正清除向量，导致PII可被恢复，提出 Epoch Key Rotation 加密删除方案能将恢复率降至 0% 且极低延迟，提供可审计身份验证。
...
### [15] [From Bits to Mixed-Radix Keys: Horner Decomposition, Uniform Sampling, and the Information-Theoretic QKD Interface of the MR-OTP](https://arxiv.org/abs/2606.18526)
*Fabio F. G. Buono*

Main category: cs.CR

TL;DR: 提出将QKD二进制熵经Horner映射转换为混合进制密钥的方法，该方法通过拒绝抽样获得均匀分布，从而在保持OTP完美机密性的同时实现信息理论安全及效率提升。


<details>
  <summary>Details</summary>
Motivation: 传统OTP仅适用于单一进制，而量子密钥分发产生的熵为二进制。需要一种将其转换为异质进制且保持完美机密性的方法。

Method: 利用Horner公式及其逆映射，将二进制整数与混合进制元组对应；识别简单模数化产生偏差的缺陷；使用拒绝抽样实现均匀性；构造批量提取器、单/多会话安全分析；研究基底恢复问题。

Result: 证明了拒绝抽样能在期望成本最优条件下恢复均匀性；提供了单/多会话信息理论安全证明；量化了效率提升；给出了基底恢复问题的无条件与有条件结果。

Conclusion: 本研究通过将原始QKD二进制熵转换为均匀混合进制密钥，保持了OTP的完美机密性，提供了一种可实现、无偏的密钥生成方法，并通过拒绝抽样恢复均匀分布，实现了信息理论安全保障。
...
### [16] [Evaluating Prompting-Based Defenses Against Domain-Camouflaged Injection Attacks](https://arxiv.org/abs/2606.18530)
*Aaditya Pai*

Main category: cs.CR

TL;DR: 改写 Prompt 是对域伪装注入攻击最可靠的防御，推荐在多模型多域部署中使用。


<details>
  <summary>Details</summary>
Motivation: 传统的注入检测依赖语法标记，无法识别使用域特定词汇隐藏的攻击指令。需寻找可降低攻击成功率的防御架构，以提升模型安全性。

Method: 在Claude Haiku、Llama 3.1 8B、Gemini 2.0 Flash 三大模型及财务、法律、通用三域中，对 5 种基于提示的防御措施（Spotlighting、Paraphrasing、Prompt Sandwiching、两种组合）进行 3,510 次试验，测定攻击成功率。

Result: 改写式防御在所有模型上均优于其他方案，攻击成功率下降幅度 55–84%。Spotlighting 在 Claude Haiku 上有效但无效于 Llama 3.1 8B。财务域残留风险最高，弱模型难以通过提示防御完全消除威胁。

Conclusion: 在对域伪装注入攻击的评估中，先对检索内容进行改写（Paraphrasing）是最稳健的防御手段，可将攻击成功率降低 55-84%。其效果始终优于 Llama Guard 4，且可作为不同模型的首选策略。
...
### [17] [AI Sandboxes: A Threat Model, Taxonomy, and Measurement Framework](https://arxiv.org/abs/2606.18532)
...
### [18] [Confident yet Concerned: Inconsistencies in Computing Students' Attitudes on Cybersecurity](https://arxiv.org/abs/2606.18541)
*Victor Adama,Robert Biddle,Nalin Arachchilage,Danielle Lottridge*

Main category: cs.CR

TL;DR: Studying 236 computing students shows mixed cybersecurity confidence and practice, with a split between knowledgeable but misled individuals and those lacking support; self‑efficacy is key to preparedness.


<details>
  <summary>Details</summary>
Motivation: To understand tech‑savvy young adults’ attitudes, knowledge, and behaviour around cybersecurity, specifically their response to deceptive online communication and phishing.

Method: Survey of 236 computing university students with thematic analysis of open-ended responses and quantitative analysis (cluster analysis, correlations).

Result: Students had decent awareness yet had incorrect beliefs, learned safety externally, lacked adequate help and were victims of cybercrime, and displayed varied confidence levels; two attitude clusters were identified, with self‑efficacy predicting preparedness.

Conclusion: Computing students possess cybersecurity awareness but show inconsistencies in practice and support utilisation, highlighting a divide between knowledge and confidence.
...
### [19] [The Gate Is Only as Honest as Its Contracts: ContractGuard for the Contract Layer of Risk-Aware Causal Gating](https://arxiv.org/abs/2606.18550)
...
### [20] [MIDS: Detecting Stealthy Masquerade and Tampering Attacks on CAN Bus via Bidirectional Mamba](https://arxiv.org/abs/2606.18599)
...
### [21] [Code-Augur: Agentic Vulnerability Detection via Specification Inference](https://arxiv.org/abs/2606.18619)
...
### [22] [TGCM: Topic-Guided Generative Disentanglement of Interleaved APT Technique Sequences](https://arxiv.org/abs/2606.18651)
...
### [23] [Understanding and Mitigating Prompt Leaking Attacks in Real-World LLM-Based Applications](https://arxiv.org/abs/2606.18673)
*Yong Yang,Chong Fu,Tong Zhang,Rui Zeng,Qingming Li,Tianyu Du,Zonghui Wang,Shouling Ji,Wenzhi Chen*

Main category: cs.CR

TL;DR: 大模型应用普遍易泄漏提示；现有防御不足；本文发现注意力漂移；提出AREA，效果好、效率高、可商用。


<details>
  <summary>Details</summary>
Motivation: 系统提示本质上是开发者的知识产权，泄漏导致安全与商业风险，现有防御失效，亟需更可靠、可用的解决方案。

Method: 对1200个商业LLM应用进行实测，结合注意力机制的机制分析，识别“注意力漂移”现象，随后设计并验证AREA防御。

Result: 80%以上的部署存在提示泄漏；AREA在保持泄漏抵抗的同时提高平均可用性33%，优化成本下降约三倍，并促使两家厂商将其视为中危漏洞。

Conclusion: 本文揭示了大规模语言模型应用中系统提示泄露的普遍性，并提出效率更高、易用性更好的AREA防御方案，显著提升泄露抵抗力和用户体验。
...
### [24] [Image Prompt Reconstruction Attacks on Distributed MLLM Inference Frameworks](https://arxiv.org/abs/2606.18710)
*Xinjian Luo,Hongyan Chang,Jianxin Wei,Yuncheng Wu,Xiaofeng Gao,Meikang Qiu,Ting Yu,Xue Liu*

Main category: cs.CR

TL;DR: 本文首次系统评估分布式多模态 LLM 中图像嵌入泄露，提出两种高效重构攻击，并在主流模型上验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有研究已关注文本嵌入泄露，然而多模态 LLM 使得图像提示在分布式推理中同样具备高度隐私敏感，且此类攻击尚未系统探究。

Method: 通过分析图像像素到中间表示的信息流，先设计高精度图像嵌入提取算法；随后构造两种被动黑盒攻击：MPAA 采用基于补丁的像素级重构，IEDA 通过嵌入引导的扩散生成进行语义级重构。

Result: 在 Gemma 3、Phi 4 Multimodal、Qwen 2.5 VL 与 Llama 4 Scout 等四大 MLLM 系列上，MPAA 与 IEDA 在多种设置下均实现了卓越的重构效果；对 MoE 架构、图像预处理、模型规模及文本‑图像互依度的进一步分析验证了攻击的普适性与对参数的敏感度。

Conclusion: 本研究首次揭示分布式多模态 LLM 框架中图像嵌入可泄露隐私的严重风险，并提出两种有效的无监督图像重构攻击（MPAA 和 IEDA）。
...
### [25] [A Predictive Neural Network Architecture for Early Detection of Low-Rate Cyberattacks](https://arxiv.org/abs/2606.18771)
*Mert Nakıp*

Main category: cs.CR

TL;DR: IDQS融合神经网络预测和差异评估，能高效且准确地检测低速拒绝服务攻击，适合资源受限的IoT部署


<details>
  <summary>Details</summary>
Motivation: 低速DDoS攻击因其隐蔽和持续性难以被传统入侵检测系统捕获，亟需轻量、前瞻性的检测方法以保护IoT网络的安全

Method: 采用RTP-QoS（基于循环趋势预测神经网络的QoS预测模型）与PDM（成对决策模型）的双层框架，对预测QoS与实际QoS之间的偏差进行评估，从而识别潜在攻击

Result: 在SDN-SlowRate-DDoS和CIC-IDS2017数据集上分别实现约79%和91%的检测准确率，高召回率、低误报，且端到端推理时间仅为0.28秒

Conclusion: IDQS 能够在资源受限的IoT环境中实现实时、轻量级的低速拒绝服务(DDoS)攻击早期检测，展示了高检测准确率与低延迟的兼顾
...
### [26] [TRAP: Benchmark for Task-completion and Resistance to Active Privacy-extraction](https://arxiv.org/abs/2606.18996)
*Moon Ye-Bin,Nam Hyeon-Woo,Baek Seong-Eun,Yejin Yeo,Tae-Hyun Oh*

Main category: cs.CR

TL;DR: 在处理敏感信息的智能体场景中，传统prompt防御无法避免泄露；通过哈希化私有字段能够在保持准确率的前提下有效减缓泄露。


<details>
  <summary>Details</summary>
Motivation: 在需要处理敏感私人信息的文档密集工作流中，智能体必须在完成任务时隐蔽地使用这些信息，但又不能让它们在响应中泄露。

Method: 提出TRAP评估指标，评测22种模型在任务准确率与隐私泄露之间的权衡，并对基于prompt的软约束防御进行实验。随后提出结构化私有字段隔离（hash key替代）来防止泄露。

Result: 所有模型均存在一定泄露率，指令跟随能力与泄露率正相关。软约束防御可降低泄露但显著牺牲任务准确率。软max模型无法实现高准确率且零泄露。结构化隔离方法在保持任务准确率的同时显著降低泄露。

Conclusion: 在软max模型锁定的前提下，使用soft constraint无法完成隐私与准确性的双赢。结构化私有字段隔离提供了一种可行方案。
...
### [27] [Lifecycle-Aware Dynamic Analysis for Secure ML Model Execution](https://arxiv.org/abs/2606.19023)
*Gabriele Digregorio,Marco Di Gennaro,Francesco Pastore,Stefano Zanero,Stefano Longari,Michele Carminati*

Main category: cs.CR

TL;DR: Re‑Moat通过动态生命周期感知检测所有模型攻击，误报极低，证明动态分析在ML模型安全中的有效性。


<details>
  <summary>Details</summary>
Motivation: 预训练ML模型越来越被恶意嵌入攻击，传统静态扫描难以通用且易漏新型攻击，需要新的安全机制。

Method: 基于对模型生命周期的定义，将攻击视为宿主系统交互异常，构建Moat框架并实现Re‑Moat进行动态监控与分析。

Result: 对Hugging Face Hub等大型数据集及31个POC模型进行评测，Re‑Moat检测全部攻击类，误报率接近零，优于现有扫描方案。

Conclusion: 验证了其通过生命周期感知的动态模式可对已知和未知攻击类实现高检出率与极低误报率，强调了动态分析在ML模型执行安全中的必要性。
...
### [28] [PYPILINE: Malicious PyPI Package Detection via Suspicious API Knowledge and Agent Workflow](https://arxiv.org/abs/2606.19063)
...
### [29] [Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage](https://arxiv.org/abs/2606.19076)
...
### [30] [Quantifying Compromise Risk in Exceptional Access Architectures Under Sparse and Indirect Evidence](https://arxiv.org/abs/2606.19106)
*Alan Woodward*

Main category: cs.CR

TL;DR: 本文通过经验分析与贝叶斯模型，系统评估 EA 系统风险，发现任何 EA 设计均高风险，OTT‑EA 在大用户基数下尾部风险尤其高。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在填补缺乏公开数据导致的评估 EA（例外访问）系统系统性风险的空白，并区分不同架构（T-EA 与 OTT-EA）在加密密钥与对数关系上的结构差异。

Method: 构建不确定性框架，采用四层分析：三层经验支柱（历史类比、Monte Carlo 场景层、渠道无关分解）与并行子图攻击图的贝叶斯结构风险模型。

Result: ①任何具备 EA 的体系架构所面临的风险都高于无 EA 的对照组；②两类架构风险分布形状不同，T‑EA 以中心趋势为主，OTT‑EA 以尾部相关活动为主；③在校准条件下，T‑EA 年化危害概率介于 1.4%–12.9%。多年度累计危害显著且不可逆的密钥泄露对 OTT‑EA 大用户群体影响更大。

Conclusion: EA 系统本质上提升了系统被破坏的概率，且 OTT‑EA 在大规模用户环境中面临更高的尾部风险。框架量化了危害概率，但未涉及后续的后果与收益评估。
...
### [31] [Giskard : Byzantine Robust and Confidential Aggregation for Large-Scale Decentralized Learning](https://arxiv.org/abs/2606.19129)
*Ousmane Touat,César Sabater,Mohamed Maouche,Sonia Ben Mokhtar*

Main category: cs.CR

TL;DR: Giskard 在可扩展性、保密性和拜占庭鲁棒性之间实现平衡，适合大规模去中心化学习场景。


<details>
  <summary>Details</summary>
Motivation: 传统方案要么需要全对全通信、计算量高；要么将计算交给少数节点导致负载不均。需要一种既能保持信息保密又能抵御拜占庭攻击、且可扩展到百万级参与者的协议。

Method: 采用以对数规模委员会为单位的树式结构，在每层委员会内部通过基于二分搜索的分布式计算实现多维协调近似中位数；整体联合作业采用安全多方计算保证保密性。

Result: 理论上证明了协议的安全与保密性；实验上对比竞品，Giskard 在每方通信复杂度上取得渐进式改进，并在存在最多四分之一拜占庭节点的情况下保持与竞品相近的模型实用性。

Conclusion: Giskard 通过将参与者组织成对数规模委员会树并在每个委员会内使用 BGW 风格的 MPC，以坐标级近似中位数实现了保密且能抵御拜占庭攻击的分散聚合，显著降低了每方通信复杂度。
...
### [32] [OpenAnt: LLM-Powered Vulnerability Discovery Through Code Decomposition, Adversarial Verification, and Dynamic Testing](https://arxiv.org/abs/2606.19149)
...
### [33] [PhantomSkill: Malicious Code Injection in Agent Skill Ecosystems](https://arxiv.org/abs/2606.19191)
*Yu-Ting Lin,Chia-Mu Yu*

Main category: cs.CR

TL;DR: 通过将恶意脚本改写为看似普通的易受攻击实现，PhantomSkill 能有效躲避 LLM 代码代理与安全审查，提示需加强资源级审计与运行时安全防护。


<details>
  <summary>Details</summary>
Motivation: LLM 代码代理通过技能获取第三方包的域特定能力，但这也为供应链攻击打开新面孔。需要研究如何隐藏恶意行为，以诱骗代理与审查者。

Method: 提出 PhantomSkill 攻击框架，使用 VulMask 技术将明显恶意脚本改写为具有漏洞形态的实现，仅在攻击者控制的触发条件下激活恶意行为，隐藏在技能的辅助资源中。

Result: 在多种主机技能、攻击目标、代码代理、生成模型和自动审查者上，VulMask 在维持正常功能的同时，显著降低警告次数和恶意软件检测率，相比完全恶意脚本更难被发现。

Conclusion: 技能生态系统需要在资源级进行审查、执行时隔离，并对技能中可利用的漏洞视为潜在恶意负载制定安全政策。
...
### [34] [CodeSentinel: A Three-Layer Defense Against Indirect Prompt Injection in Code Contexts](https://arxiv.org/abs/2606.19235)
...
### [35] [Gaussian Mixture Attention: Linear-Time Sequence Mixing via Probabilistic Latent Routing](https://arxiv.org/abs/2606.18283)
...
### [36] [Breaking the Solver Bottleneck: Training Task Generators at the Learnable Frontier](https://arxiv.org/abs/2606.18284)
...
TL;DR: PROPEL通过训练探针评估解算率，加速并提升生成可解任务的比例。


<details>
  <summary>Details</summary>
Motivation: 固定任务分布饱和，直接评估生成器难以实现，需大规模求解器回滚而耗时。

Method: 使用一键标记的任务与解算器结果语料训练轻量激活探针，探针预测解算器通过率，代替多次求解器回溯，使生成器优化仅需一次前向传播。

Result: 在Qwen2.5系列求解器下，学习前沿任务通过率从10.1%提升至20.0%；在Qwen3.5-27B软件工程任务中，通过率从9.8%提升至19.6%。

Conclusion: PROPEL通过代理预测提高任务生成器的学习前沿任务比率，在多模型规模上显著提升代码和软件工程任务的可解决率。
...
### [37] [CODEBLOCK: Learning to Supervise Code at the Right Granularity](https://arxiv.org/abs/2606.18286)
...
### [38] [Artemis: Anatomy-Resolved inTervention for Eliminating Multimodal NeuroImage confounderS](https://arxiv.org/abs/2606.18287)
...
### [39] [A Link between Shock-wave Theory and Symmetry-reduced Stochastic Gradient Descent for Artificial Neural Networks](https://arxiv.org/abs/2606.18303)
*Taiki Miyagawa*
...
### [40] [Attribution-Guided and Coverage-Maximized Pruning for Structural MoE Compression](https://arxiv.org/abs/2606.18304)
*Yifu Ding,Jiacheng Wang,Ge Yang,Yongcheng Jing,Jinyang Guo,Xianglong Liu,Dacheng Tao*

Main category: cs.LG

TL;DR: MoE 模型内存大、推理慢；传统专家级剪枝太粗。本文发现精细通道冗余，提出通道得分覆盖最大化剪枝框架；实验显示可在大幅压缩下保持准确率，显存压缩近5倍，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: Mixture-of-Experts 模型在扩展计算能力时虽然效率高，但因显著的内存占用和推理开销，部署成本仍然很高。传统压缩方法大多在专家级别操作，去除整体专家或按粗粒度重要性评分排序，但这类决策过于粗糙，难以捕捉细粒度冗余，导致剪枝预算分配不合理且压缩效果受限。

Method: 我们观察到 MoE 专家的信息高度集中在少量通道上，即使在被认为重要的专家中也仍存在大量冗余。因此提出了针对 MoE 的结构化剪枝框架：将剪枝比例分配重新表述为通道得分覆盖最大化问题，并用基于归因的近似方法高效求解。

Result: 在 DeepSeek 与 Qwen MoE 模型上，结合 4‑bit 定量化时，方法在 50% 或 25% 的结构化剪枝下均保持模型准确率。对 Qwen3‑30B‑A3B 的实验表明，显存占用可降低 5.27×，且在多项基准测试中持续优于现有最先进基线。

Conclusion: 通过细粒度通道级别的结构化剪枝，即使在保留 4‑bit 定量化的前提下，也能显著减少 MoE 模型的存储需求和推理成本，同时保持甚至提升性能，展示了通道覆盖优化在专家网络压缩中的有效性。
...
### [41] [Fisher Width: A Geometric Measure of Complexity on Statistical Manifolds](https://arxiv.org/abs/2606.18306)
*Vu Khac Ky*
...
### [42] [DRIFT: Refining Instruction Data via On-Policy Data Attribution](https://arxiv.org/abs/2606.18307)
*Zefan Wang,Lincheng Li,Tianyu Yu,Yuan Yao*

Main category: cs.LG

TL;DR: DRIFT通过在策略内做验证、加权修正和去偏，解决了传统IF在SFT中的两大局限，最终在7B模型上提升性能。


<details>
  <summary>Details</summary>
Motivation: 目前针对LLM的SFT训练，只关注在预算内加速训练，未能提高能力上限。本研究的目标是从实例层面优化数据分布，以提升最终模型性能。

Method: 利用影响函数（IF）进行实例级数据归因，并针对其两大限制（离政策验证目标的近似误差及梯度范数偏差）提出DRIFT：使用模型本身的on-policy rollouts作为验证目标；对轨迹正确性进行符号加权；去除梯度放大偏差；最后以少量验证查询为锚点，赋予整个数据集可靠归因。

Result: 在7B参数的指令与推理模型上实验，DRIFT在提升性能上限方面持续优于现有数据策划基线。

Conclusion: 设计并验证了一种通过on-policy影响函数进行数据精炼的方法，能有效提升LLM的最终性能。
...
### [43] [TRIDENT: Breaking the Hybrid-Safety-Physics Coupling for Provably Safe Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.18308)
*Zijie Meng,Ziwei Li,Yufei Liu,Zhiyu Li,Jiyuan Liu,Wenhua Nie,Bingcai Wei,Miao Zhang*

Main category: cs.LG

TL;DR: TRIDENT通过梯度修正、Lyapunov约束与物理残差评论，解决安全多智能体学习中的偏差循环，实现显著低违规率与高奖励。


<details>
  <summary>Details</summary>
Motivation: 在联网的复杂网络物理系统中，需要学习算法同时管理离散-连续混合操作、训练时的严格安全约束以及受物理规律约束的动力学，传统模块化方法易陷入相互作用的偏差循环。

Method: 提出TRIDENT框架：1) Richardson-Romberg梯度修正降低Gumbel-Softmax偏差；2) Lyapunov约束的顺序收缩更新强制逐步可行；3) 物理信息残差评论在价值而非奖励上分解。通过三向耦合定理保证三项共设计消除泄漏。

Result: 证明O~(1/√K)收敛率至受限纳什均衡，累计违规O(√K)。在多UAV移动边缘计算、自动交叉口管理与Hybrid SMAC实验中，TRIDENT将训练违规率分别降低95.5%（MADDPG）和76.3%（MACPO），同时相比最佳无约束基线提升13.5%奖励。

Conclusion: 证明了三向耦合定理对安全多智能体学习的理论与实践价值，TRIDENT在安全约束下实现更高效更稳健的学习。
...
### [44] [SAGE: Retain-Aware Post-Hoc Sanitization of Final Unlearning Vector](https://arxiv.org/abs/2606.18309)
...
### [45] [Ghost Attractor Networks: Basin-Structured Dynamical Decoders for Closed-Loop Sequential Generation](https://arxiv.org/abs/2606.18315)
...
### [46] [A Survey on Data-Driven Models for Soil Moisture Regression and Classification](https://arxiv.org/abs/2606.18316)
...
### [47] [Enhanced Graph Neural Networks using K-Hop Gaussian Diffusion](https://arxiv.org/abs/2606.18317)
...
### [48] [Beyond AHI: An Interpretable Causal-Discovery-Guided Framework for Sleep Recovery in Connected Health](https://arxiv.org/abs/2606.18506)
...
### [49] [ASTRA: A Scalable Next-Generation ATCO Training Simulator with Autonomous Simpilots](https://arxiv.org/abs/2606.18319)
...
### [50] [FoMoE: Breaking the Full-Replica Barrier with a Federation of MoEs](https://arxiv.org/abs/2606.19025)
...
### [51] [SAE Interventions are Unreliable: Post-Intervention Recovery of Suppressed Behavior](https://arxiv.org/abs/2606.18322)
*Mingyue Cui,Linghui Shen,Xingyi Yang*

Main category: cs.LG

TL;DR: SAE特征干预可阻挡可视路径，但模型能通过残差重构恢复行为，表明特征控制并不足以完全约束安全相关行为。


<details>
  <summary>Details</summary>
Motivation: 评估现有基于SAE的潜在空间防御中，特征级干预是否真的能够完全阻止安全风险行为，揭示可能的恢复漏洞。

Method: 提出针对残差空间的post-intervention recovery优化，利用编码器正交更新及跨层特征映射雅可比，构造在干预仍在的情况下恢复预干预行为的扰动。

Result: 在TPP、unlearning、IOI及拒绝导向实验中，发现即使ICF/SAE特征被clamp，仍可实现高达95.8% 的恢复率，且相对漂移保持低于0.131；归因分析显示恢复主要源自未解释的SAE重构残差。

Conclusion: 尽管通过不安全的SAE特征进行窒断可以抑制明显的行为路径，但模型仍能通过恢复路径维持原有行为，表明特征层面的干预并不能保证行为完全受控。
...
### [52] [OrthoReg: Orthogonal Regularization for Hybrid Symbolic-Neural Dynamical Systems](https://arxiv.org/abs/2606.19145)
...
### [53] [Domain-Shift Aware Neural Networks for Unbalance Characterization in Rotating Systems](https://arxiv.org/abs/2606.18882)
...
### [54] [Why SWAVE May Not Be All You Need:A Concept-Evolution Retrospective on Complex-Valued Recurrent Language Models](https://arxiv.org/abs/2606.18324)
...
### [55] [Neural Network Implementation of the Renormalization Group for Fault Diagnosis with Class Imbalance](https://arxiv.org/abs/2606.18326)
*Evgeny Nikulchev,Dmitry Ilin*

Main category: cs.LG

TL;DR: RGNet 通过重整化群层次压缩特征空间，在不平衡故障预测任务中取得优异表现并具可解释性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在实际任务中常面临类别不平衡与多维噪声，传统方法难以兼顾局部细节与全局模式的捕捉。

Method: 提出基于重整化群（Renormalization Group）思想的 RGNet 网络，逐级压缩输入维度并在最终分类前将所有尺度信息拼接，形成层次化的粗化表示；通过 t‑SNE 可视化 RG‑flows，可解释低维表征。

Result: 在 AI4I 公开的不平衡数据集上实验，RGNet 在故障预测任务中表现优异，超越或与现有方法同等竞争，且模型结果可解释。

Conclusion: RGNet 为不平衡分类提供了一种可解释且性能稳定的通用解决方案，可适用于多种实际任务。
...
### [56] [Self-CTRL: Self-Consistency Training with Reinforcement Learning](https://arxiv.org/abs/2606.18327)
*Itamar Pres,Laura Ruis,Melat Ghebreselassie,Belinda Z. Li,Jacob Andreas*

Main category: cs.LG

TL;DR: Self-CTRL通过强化双向一致性训练，提升模型自我解释的准确性和行为一致性，从而显著加强安全、透明和可控性。


<details>
  <summary>Details</summary>
Motivation: 为了解决语言模型难以透明可审计的缺陷，提升用户对模型行为的信任与理解。

Method: Self-CTRL利用强化学习实现自我解释与行为的双向一致性训练：在一致性方向上更新解释使其更好预测行为，反之亦然，形成自我强化循环。

Result: 在概率推理任务中，自我一致性训练使自报偏差与真实偏差的 $R^2$ 从0.24提升至0.64；在宪法式 AI 约束任务中，模型对拒绝与遵从的规则预测准确率从36%提升至92%，同时模型在对有害内容的识别上的错误率降至0.5%。

Conclusion: 该研究通过将模型的自我解释与实际行为对齐，显著提升了模型的可解释性、安全性与可控性。
...
### [57] [ThousandWorlds: A benchmark for climate emulation of potentially habitable exoplanets](https://arxiv.org/abs/2606.18338)
*Edward T. Stevenson,Mei Ting Mak,Eric Wolf,Denis E. Sergeev,Tobi Hammond,N. J. Mayne,Miles Cranmer*

Main category: cs.LG

TL;DR: 提出ThousandWorlds数据集，含1800多模拟、三套难度子集及两评估协议；GP模型最突出，提示该领域仍有研究空间。


<details>
  <summary>Details</summary>
Motivation: 为了有效识别外星行星大气中的微弱生物学标志，需要准确理解其气候；传统GCM运行耗时高昂，机器学习逼真模型可取代但缺乏多磁模型的数据集。

Method: 首先构建1800多条模拟数据集，涵盖五种全球气候模型的8个行星参数到3D大气场；再设计三层子集——单模拟器、完整多模拟器、结构性缺失多模拟器；随后提出两套评估协议：按方法排名和按GCM间差异衡量性能；最后对七种基线（简单方法、深度学习、Gaussian process）进行评估。

Result: Data set 构建完成；GP方法在所有子集上表现最佳，表明目前深度学习在此“低数据、多模拟器”的任务中尚未突破。

Conclusion: ThousandWorlds为低数据、多模拟器的参数到场（parameter-to-field）回归提供了基准，结果表明高斯过程方法目前优于深度学习，提示偏好单一GCM时的深度方法仍难以取得最佳性能。
...
### [58] [Do Time Series Foundation Model Benchmarks Hide Regime-Dependent Failures? Evidence from Traffic Speed Forecasting](https://arxiv.org/abs/2606.18367)
...
### [59] [From Sparse Features to Trustworthy Proxies: Certifying SAE-Based Interpretability](https://arxiv.org/abs/2606.18383)
*Dibyanayan Bandyopadhyay,Asif Ekbal*

Main category: cs.LG

TL;DR: 作者提出后验推广框架，用四个可测量量求得语言模型的稀疏代理上界，验证后该上界可非空，并通过层级与特征混洗实验展示其可解释性可信度与模型行为的紧密关联。


<details>
  <summary>Details</summary>
Motivation: 稀疏自动编码器常用于从语言模型中提取可解释特征，但尚不清楚这些解释是否真实可靠；本研究旨在构建一种可量化、可检验的可信度度量，以判断 SAE 解释对底层冻结模型的忠实度。

Method: 采用后验推广框架，将冻结的语言模型替换为基于预训练 SAE 重建的稀疏代理；通过计算四个可测量量（代理风险、重建误差、概念池不匹配、稀疏复杂度）得到基体模型期望风险的上界，并将其作为可解释性可信度准则；随后在若干大型模型上进行实证验证，并进行层级与特征混洗消融实验。

Result: 在 GPT‑2 Small、Gemma‑2B 与 Llama‑3‑8B 上，使用实际样本量即可得到非空上界，显示提取的稀疏特征保留了有效预测信息；层级分析表明后深层更易被证明，且当地度量更强、下游错误放大更弱；特征混洗实验证明框架能区分语义关联与仅统计稀疏，成为判断 SAE 解释可靠度的有用诊断工具。

Conclusion: 本文提出的后验推广框架可为稀疏自动编码器（SAE）提供可衡量的可信度上界，证明在 GPT‑2 Small、Gemma‑2B 与 Llama‑3‑8B 等模型上，足够多的样本可使上界非空，从而确认从 SAEs 提取的稀疏特征确实携带有意义的预测信息，并能保持与原模型的行为接近；层级分析显示后深层更易证明；特征混洗实验进一步验证了其区分语义对齐与纯统计稀疏的能力。
...
### [60] [SCOPE-FL: A Strategy-proof Chain-based Optimal pareto efficient Federated Learning System](https://arxiv.org/abs/2606.18384)
...
### [61] [LLMZero: Discovering Adaptive Training Strategies for RL Post-Training via LLM Agents](https://arxiv.org/abs/2606.18388)
...
### [62] [MOLAR: Learning Multimodal Molecular Representations from Noisy Labels](https://arxiv.org/abs/2606.18390)
...
### [63] [P$^2$CE: Model-Agnostic Plausible Pareto-Optimal Counterfactual Explanations](https://arxiv.org/abs/2606.18418)
...
### [64] [Measurement noise limits the advantage of nonlinear models over linear models in biomedical prediction](https://arxiv.org/abs/2606.18420)
...
### [65] [Signature filtering: a lightweight enhancement for statistical watermark detection in large language models](https://arxiv.org/abs/2606.18430)
...
### [66] [Beyond Prediction: Tail-Aware Scheduling for LLM Inference](https://arxiv.org/abs/2606.18431)
*Yueying Li,Yuanfan Chen,Jiayang Chen,Esha Choukse,Haoran Qiu,G. Edward Suh,Rodrigo Fonseca,Ziv Scully,Udit Gupta*

Main category: cs.LG

TL;DR: 通过软优先级提升和缓存感知预emption，无需长度预测即可显著降低LLM推理的尾部延迟，提供了鲁棒的在线服务优化方案。


<details>
  <summary>Details</summary>
Motivation: 现有基于长度预测的SJF/SRPT近似调度在长度变异性大、分布漂移、峰值到达和GPU内存压力下表现脆弱，且难以控制TTFT和P90-99等尾部延迟关键指标。

Method: 构建了分布感知、轻量统计信号驱动的软优先级提升机制，并将其与缓存感知预emption协同优化，以实现对LLM推理时序和内存耦合动态的精准调度。

Result: 在生产与公开轨迹评估中，相较于具有完美长度知识的SRPT，P99 TTLT减少35-50%，TTFT减少34-47%，且在推理需要推理沉重和聊天繁重的任务上均表现优异。

Conclusion: 提出了一种无需长度预测、基于分布的软优先级与内存感知预emption的LLM调度框架，在多种工作负载下显著降低尾部延迟，展现出对分布变化和GPU内存压力的鲁棒性。
...
### [67] [TMR-GGNN: Credit Card Fraud Detection based on Time-Aware Multi-Relational Guided Graph Neural Network](https://arxiv.org/abs/2606.18444)
*Rohit Tewari,Shubhankar Shilpi,Navin Chhibber,Devendra Singh Parmar,Sunil Khemka,Piyush Ranjan*

Main category: cs.LG

TL;DR: 通过构造时间感知多关系图网络并结合 InfoNCE+Focal 损失，TMR‑GGNN 在信用卡欺诈检测中显著提升了少数类样本识别效果。


<details>
  <summary>Details</summary>
Motivation: 信用卡欺诈检测面临数据严重不平衡、欺诈模式快速变化以及交易实体之间复杂多关系的问题，需要一种能同时考虑时间、关系和对比学习的模型。

Method: 采用扩展的编码‑解码图神经网络框架，构造多关系动态图，使用时间感知关系注意机制对交易进行加权，并在解码器中通过对比学习区分真实与合成交易，随后采用 InfoNCE 与 Focal 损失的组合来提升模型对稀有欺诈样本的判别能力。

Result: 实验表明，TMR‑GGNN 在多种真实电商与支付数据集上相较传统模型取得更高的召回率与更低的误报率，尤其在极端样本不平衡时的表现更为显著。

Conclusion: 该研究提出的 Timeaware Multi Relational Guided Graph Neural Network（TMR‑GGNN）能够显著提升信用卡欺诈检测的准确性，特别是在处理高度不平衡数据、不断演变的欺诈模式以及复杂的交易实体关系方面表现优异。
...
TL;DR: SWAAP利用双层优化确定低回报目标，然后通过梯度匹配精细投毒模型训练数据，实现在保持数据接近干净分布的前提下，显著削弱世界模型的规划性能。


<details>
  <summary>Details</summary>
Motivation: 在模型基学习中，世界模型的更新步骤为攻击提供了攻击面，攻击者能够利用投毒的微调轨迹操纵学习到的动力学，破坏随后的规划过程。

Method: SWAAP先使用一阶双层优化与迁移梯度定理识别一个具有低回报但近似干净动力学的目标世界模型；随后通过受限梯度匹配对有限比例的微调转移目标进行精细修改，使训练梯度引导模型接近该攻击目标，同时预测误差正则化保证被污染的样本与自然误差相近。

Result: 实验评估表明，SWAAP在多种连续控制任务中能够显著降低绩效，同时生成的被污染转移样本与干净数据非常相近，能够规避预训练检测、鲁棒训练以及测试时监测等常规防御方法。

Conclusion: 该论文揭示了基于世界模型的自适应学习系统存在训练时攻击的脆弱性，通过两阶段的数据投毒方法能够在不显著改变模型表现的前提下导致规划性能明显下降。
...
### [69] [A Cross-Model VLM-Judge Protocol for Single-Image 3D Mesh Quality (and Why Cheap Proxies Fall Short)](https://arxiv.org/abs/2606.18451)
...
### [70] [Veriphi: Attack-Guided Neural Network Verification with Dataset-Dependent Training Methods](https://arxiv.org/abs/2606.18454)
...
### [71] [Task-Restricted Symmetries in Recurrent Weight Space](https://arxiv.org/abs/2606.18457)
*Simon Dräger*

Main category: cs.LG

TL;DR: 使用Schur坐标分解研究RNN权重冗余，发现任务相关的可消融耦合模式，提供判断网络稳健性的诊断方法。


<details>
  <summary>Details</summary>
Motivation: 研究循环网络权重空间功能冗余，探究其对任务性能的影响以及潜在的函数不变性。

Method: 在单层tanh RNN上使用有序实Schur分解，将谱块与奇异耦合分离。对输入与读出保持不变，进行结构化消融实验。

Result: 发现某些非正规Schur耦合可被移除而对固定长度复制任务几乎不影响性能，而对自律重放等其他任务仍必需。不同任务及训练方案的消融特征各异，揭示了近似的功能不变量而非普适对称。

Conclusion: Schur坐标消融提供了一种实用诊断工具，能判断哪些结构化扰动可保持已训练的循环计算，哪些会破坏其功能。
...
### [72] [What Does the Weight Norm Control in Grokking? Logit-Scale Mediation under Cross-Entropy](https://arxiv.org/abs/2606.18465)
*Truong Xuan Khanh*

Main category: cs.LG

TL;DR: grokking延迟主要由logit尺度决定，权重范数仅是间接手段。


<details>
  <summary>Details</summary>
Motivation: 探究grokking延迟背后真正被控制的变量，验证权重范数是否仅是间接因素。

Method: 通过在保持权重范数不变、只调节输出温度的条件下，改变交叉熵损失，滑动grokking延迟；随后在不同范数和温度网格中统计延迟并回归；并对不同损失（MSE）和模型类型（无 LayerNorm transformer、softmax 归一化）进行实验验证。

Result: logit尺度几乎完全决定延迟（R^2=0.97），权重范数仅在此基础上增加1-2%的解释贡献；在MSE损失下范数通过不同路径起作用。

Conclusion: 权重范数对grokking延迟的影响主要表现在对logit尺度的控制；logit尺度是决定延迟的主要因素，范数只是上游手段。
...
### [73] [Structured Representation Learning with Locally Linear Embeddings and Adaptive Feature Fusion](https://arxiv.org/abs/2606.18469)
*Somjit Nath,Jackson J Cone,Derek Nowrouzezahrai,Samira Ebrahimi Kahou*

Main category: cs.LG

TL;DR: 基于神经科学启发的RL框架，使用LLE和注意融合，实验表明更高效、更好性能


<details>
  <summary>Details</summary>
Motivation: 指出神经科学发现之启示

Method: 利用LLE和注意力的RL方法

Result: 相较传统RL提升学习效率与表现

Conclusion: 强调本方法捕捉局部状态结构与自适应特征选择
...
### [74] [The Illusion of Improvement: Reject Inference Strategies in Credit Scoring](https://arxiv.org/abs/2606.18479)
...
### [75] [SFT Overtraining Predicts Rank Inversion via Entropy Collapse Under RLVR](https://arxiv.org/abs/2606.18487)
*Siddharth Aphale,Kelly Liu*

Main category: cs.LG

TL;DR: 深度SFT可提升pass@1，但会压缩rollout分布并削弱GRPO性能。预RL熵是风险指标，通过两阶段诊断可提前终止失败实验，常规正则化无法恢复受损检查点。


<details>
  <summary>Details</summary>
Motivation: 标准的以pass@1最高的SFT检查点作为GRPO起点的启发式策略在SFT压缩rollout分布时会失效，需要更可靠的检查点选择方法。

Method: 对Qwen2.5-Coder-3B和DeepSeek-Coder-6.7B分别构建SFT深度梯度，分别在5/4个深度和3个随机种子下评估，记录pass@1、GRPO pass@10、预RL熵等指标；对预RL熵进行相关性分析；设计两阶段诊断流程并在实验中验证；尝试KL正则化与标签平滑作为干预手段。

Result: Qwen: 随深度增高，pre RL pass@1上升，但GRPO pass@10从0.806降至0.481；pre RL熵与GRPO结果相关系数+0.69。DeepSeek: pass@1远高于阈值，GRPO结果压缩而非反转。两阶段诊断能有效标记风险检查点。KL正则化和标签平滑无效。

Conclusion: 在SFT深度增加时，虽然pass@1上升，但GRPO的pass@10显著下降，说明传统的用最高pass@1挑选SFT检查点的做法会因rollout分布压缩而失效。预RL熵与GRPO结果高度相关，可以用来预警。两阶段诊断（预RL熵筛选+早期GRPO熵监控）可及时识别高风险检查点并停止失败实验。KL正则化和标签平滑在此场景下无法弥补Qwen模型的坍塌。
...
### [76] [Quantum Annealing Enhanced Reinforcement Learning for Accurate Remaining Useful Lifetime Prediction](https://arxiv.org/abs/2606.18503)
*Manoranjan Gandhudi,Arunkumar V.,G. R. Anil,Gangadharan G. R*

Main category: cs.LG

TL;DR: 量子退火增强Q‑学习框架对剩余寿命估计进行探索决策，显著提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 剩余寿命估算对预测性维护至关重要，而传统统计模型无法捕捉系统的强非线性，数据驱动模型在高维非凸空间中易陷入次优解。

Method: 提出将量子退火与Q‑学习相结合的QAQL框架，将每一次Q‑值更新编码为QUBO，利用量子退火器获得近似最优动作的概率分布，从而为探索提供信息，并将退火器嵌入强化学习循环。

Result: 在NASA C‑MAPSS和设备舰队保养数据集上多次独立运行并比较六种误差指标，QAQL显著优于经典与量子基准，差异具有统计显著性。

Conclusion: 量子退火可作为强化学习循环中可用的非理论优化器，在工业预测性维护中具有实际应用价值。
...
### [77] [Concept Modulation Models: A Unified Framework for Identifiability and Extrapolation](https://arxiv.org/abs/2606.18509)
...
### [78] [PSyGenTAB: A Privacy-Preserving Framework for Synthetic Clinical Tabular Data Generation via Constrained Optimization](https://arxiv.org/abs/2606.18518)
...
### [79] [Sparsity Curse: Understanding RLVR Model Parameter Space from Model Merging](https://arxiv.org/abs/2606.18521)
...
### [80] [On the Residual Scaling of Looped Transformers: Stability and Transferability](https://arxiv.org/abs/2606.18524)
...
### [81] [Hierarchical Attention via Domain Decomposition](https://arxiv.org/abs/2606.18525)
...
### [82] [Do as the Romans Do: Learning Universal Behaviors from Heterogeneous Agents](https://arxiv.org/abs/2606.18537)
...
### [83] [Effects of sparsity and superposition on loss in simple autoencoders](https://arxiv.org/abs/2606.18538)
...
### [84] [TS-Fault: Benchmarking Time Series Forecasters Against Structural Faults](https://arxiv.org/abs/2606.18539)
...
### [85] [Correcting Sensor-Induced Distribution Drift with Wasserstein Adversarial Learning](https://arxiv.org/abs/2606.18561)
...
### [86] [Fair Cognitive Impairment Detection Through Unlearning](https://arxiv.org/abs/2606.18571)
...
### [87] [Towards Anomaly Detection on Relational Data](https://arxiv.org/abs/2606.18621)
...
### [88] [PACT: Preserving Anchored Cores in Task-vectors for Model Merging](https://arxiv.org/abs/2606.18627)
...
### [89] [MetaboNet-Bench: A Multi-modal Benchmark for Glucose Forecasting in Type 1 Diabetes](https://arxiv.org/abs/2606.18640)
...
### [90] [BLADE: Scalable Bi-level Adaptive Data Selection for LLM Training](https://arxiv.org/abs/2606.18650)
...
### [91] [scGTN: Deep Siamese Graph Transformer Network for Single-cell RNA Sequencing Clustering](https://arxiv.org/abs/2606.18672)
...
### [92] [InTrain: Intrinsic Trainability for Zero-Cost Neural Architecture Search](https://arxiv.org/abs/2606.18676)
...
### [93] [Bounded Context Management for Tabular Foundation Models on Stream Learning](https://arxiv.org/abs/2606.18677)
...
### [94] [Dual-Channel Grounded World Modeling (DCGWM): Structural Prevention of Objective Interference Collapse via Heterogeneous External Grounding with Inward-Only Gradient Flow](https://arxiv.org/abs/2606.18688)
*Akshay Hazare*
...
### [95] [Robust and Interpretable Adaptation of Equivariant Materials Foundation Models via Sparsity-promoting Fine-tuning](https://arxiv.org/abs/2606.18691)
*Youngwoo Cho,Seunghoon Yi,Wooil Yang,Sungmo Kang,Young-woo Son,Jaegul Choo,Joonseok Lee,Soo Kyung Kim,Hongkee Yoon*

Main category: cs.LG

TL;DR: 通过稀疏性提升微调，使E(3)-基础模型在能量、力、磁性预测上精准且参数更新极少，具备可解释性与领域适用性


<details>
  <summary>Details</summary>
Motivation: 预训练材料模型虽强但因物理化学多样性及预训练与实际设置不匹配，需要高效、领域专用微调

Method: 利用E(3)等变性结构特征，采用稀疏性约束选择性更新模型参数

Result: 在分子和晶体基准上，所提方法仅更新约3%参数（部分情况0.5%）即可匹配或优于全参数微调和低秩适配；在磁矩预测和磁性相关能量建模中亦表现良好；稀疏模式揭示$d$轨道贡献增强等物理可解释性

Conclusion: 稀疏性促进的微调方法能够在保持参数更新量极低（约0.5-3%）的同时，使基于E(3)-等变材料基础模型在能量、力和磁性预测任务上与完全微调或低秩适配相当或更优，并且具有可解释性
...
### [96] [Attention as Frustrated Synchronization](https://arxiv.org/abs/2606.18694)
*Joshua Nunley*

Main category: cs.LG

TL;DR: FSN 在1百万参数的变压器同等条件下，在字符与代码上表现更佳，并在更多规模上保持优势。


<details>
  <summary>Details</summary>
Motivation: 开发基于同步失配的注意力架构，为计算赋能；

Method: 提出受挫同步网络(FSN)，利用相位、和谐复合耦合核与单步延迟，实现数据驱动的同步失配；

Result: 在字符级文本与代码数据上，FSN在相同参数规模与训练预算下，验证损失低于调优后的RoPE-SwiGLU变压器；在enwik8 训练、归纳性能均优于基线；

Conclusion: 受挫同步网络提供了一种无需多层感知机的高效替代方案，已在多规模参数上显示优势，且在多层次上可弥补未受挫层的性能缺口；
...
### [97] [Contextualizing Biological Language Models across Modalities via Logit-Space Contrastive Alignment](https://arxiv.org/abs/2606.18703)
*Yanjun Shao,Yundi Chen,Yashvi Patel,Aurelien Pelissier,María Rodríguez Martínez*

Main category: cs.LG

TL;DR: LOGICA通过在logit空间的对比学习实现上下文条件化预测，保持token-level接口，提升了多种生物序列设计与变异评估任务的效果。


<details>
  <summary>Details</summary>
Motivation: 预训练生物语言模型学习的概率分布来自广泛无标签语料，缺少针对任务特定生物上下文（如相互作用伙伴、细胞环境、治疗干预）的条件化；现有方法往往通过池化嵌入、对比潜在空间或任务特定预测头扭曲这一接口。

Method: LOGICA在输出logit空间进行对比学习，采用与模型本地token head兼容的门控跨模态适配器，将上下文化的token log-likelihood转换为匹配得分，直接在token概率上对齐而非共享嵌入空间。

Result: 在蛋白质-配体结合、TCR-肽活性以及药物条件化耐药预测等任务中，LOGICA均优于先前的最先进方法，包括匹配潜在对比和条件MLM基线；在单基因未见突变的药物耐药预测中，LOGICA将AUC从约0.55提升至约0.65。

Conclusion: LOGICA在保持预训练的概率接口的同时，实现了上下文条件化的预测，显著提高了多项任务的性能，尤其是在稀疏配对数据的突变位点变异评估上。
...
### [99] [Graph Grounded Cross Attention Transformer Neural Network for Structurally Constrained Full Event Sequence Generation in Predictive Process Monitoring](https://arxiv.org/abs/2606.18726)
*Fang Wang,Ernesto Damiani*

Main category: cs.LG

TL;DR: GGATN通过全局图编码+图交叉注意力+一次性多元生成+图约束解码，有效解决完整事件序列生成挑战，性能优于传统与LLM基线。


<details>
  <summary>Details</summary>
Motivation: 现有PPM研究多聚焦单项预测，缺乏统一的完整序列生成方法，同时需满足转移可行、时间顺序、终止与属性一致性等多重约束。

Method: 采用全流程图作为全局语义记忆，融合Transformer自注意力对序列位置进行上下文建模，并通过图基础交叉注意力注入流程拓扑；随后一次性预测活动、时间戳、长度及属性，并使用Viterbi式图约束解码确保可行路径和终止。

Result: 在六个基准事件日志上，GGATN在序列相似度、Damerau Levenshtein相似度、双元组控制流相似度和持续时间分布等指标上均优于基线，还实现了零幻影活动和零属性不一致。

Conclusion: GGATN通过一次性生成路径并结合图约束解码，实现了在预测过程监测任务中的可靠事件序列生成，显著提升了生成质量并消除了幻影活动和属性不一致。
...
### [100] [Low-Cost Neuromorphic Fall Detection Using Synthetic Event Data and Hybrid SNNs](https://arxiv.org/abs/2606.18732)
...
### [101] [Private Learning with Public Feature Conditioning](https://arxiv.org/abs/2606.18773)
...
### [102] [RouteJudge: An Open Platform for Reproducible and Preference-Aware LLM Routing](https://arxiv.org/abs/2606.18774)
...
### [104] [Bayesian Anytime Pareto Set Identification for Multi-Objective Multi-Armed Bandits](https://arxiv.org/abs/2606.18785)
...
### [105] [Learning from Own Solutions: Self-Conditioned Credit Assignment for Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2606.18810)
...
### [106] [Reinforcement Learning Foundation Models Should Already Be A Thing](https://arxiv.org/abs/2606.18812)
...
### [107] [Maturing Markov Decision Processes: Decision Making under Increasing Information and Shrinking Action Sets](https://arxiv.org/abs/2606.18820)
...
### [108] [GateMem: Benchmarking Memory Governance in Multi-Principal Shared-Memory Agents](https://arxiv.org/abs/2606.18829)
...
### [109] [Target-confidence Recourse Using tSeTlin machines: TRUST](https://arxiv.org/abs/2606.18832)
...
### [110] [Seed-Guided Semi-Supervised Clustering by A-Contrario Anomaly Detection](https://arxiv.org/abs/2606.18833)
*Nassir Mohammad*
...
### [111] [Identifying Structural Biases from Causal Mechanism Shifts](https://arxiv.org/abs/2606.18834)
...
### [112] [Semantic Robustness Certification for Vision-Language Models](https://arxiv.org/abs/2606.18839)
...
### [113] [Learning from Your Own Mistakes: Constructing Learnable Micro-Reflective Trajectories for Self-Distillation](https://arxiv.org/abs/2606.18844)
...
### [114] [Investigating Inductive Biases for Machine Learning Emulation of Sudden Stratospheric Warmings in Idealised Isca Simulations](https://arxiv.org/abs/2606.18857)
*Oskar Bohn Lassen,Simon Driscoll,Stephen I. Thomson,Sebastian Schemm,Francisco C. Pereira*

Main category: cs.LG

TL;DR: 三维垂直耦合对捕捉SSW至关重要，但低预测误差并不能保证物理真实性。


<details>
  <summary>Details</summary>
Motivation: 探究机器学习模拟器在子季节到季节尺度上利用平流层可预测性锚点（如SSW）扩展气象预测精度的可能性，并评估不同网络架构是否能充分捕捉平流层与对流层的时空耦合。

Method: 采用Cnn、Transformer和基于图的三种架构，对比训练时仅在高度为波-2加热扰动的理想化Isca模拟中进行一步预测。通过对比这三种网络在静态平流层与SSW相关强变动情况下的性能差异，评估它们的架构先入偏置。

Result: 在平流层相对静止时，三种架构的预测差异不大；当存在SSW类似的强波动时，差异显著扩大。显式三维耦合的架构在低误差下表现最好，但Eliassen‑Palm通量诊断显示其在波-平均流相互作用方面仍不完全物理合理，波驱动力结构仍有共振误差。

Conclusion: 该研究表明，显式三维垂直耦合作为一种结构先入偏置，能显著提升机器学习模拟突发平流层暖化（SSW）动力的能力。然而，即使在预测误差低的情况下，模型仍无法完全再现波-平均流相互作用的物理过程，波驱动力结构仍存在相干误差。
...
### [115] [Scaling Learning-based AEB with Massive Unlabeled Data](https://arxiv.org/abs/2606.18864)
...
TL;DR: 使用元反馈半监督学习和噪声门控技术，三十亿条无标签数据训练的AEB模型，在生产部署后安全提升35%，误激活率降低。


<details>
  <summary>Details</summary>
Motivation: 利用海量生产中产生的未标记行驶数据提升模型学习；在生产环境下需保障安全、控制误触，传统方法在面对锚点歧义和不匹配时会放大系统误差。

Method: 基于元反馈半监督学习的教师-学生框架；加入噪声感知解耦和基于动力学的伪标签门控以及冲突惩罚，以消除锚点模糊和标签-未标签不匹配导致的误差。

Result: 从1M到1B窗口标注数据，模型安全性持续提升；在10^9公里的验证中，正激活与误激活比>100:1，事故无事件里程提升35%。

Conclusion: 本研究通过大规模无标签车队数据、元反馈半监督学习（MF-SSL）有效提升自动紧急制动（AEB）系统性能，在保持舒适度的同时显著提高安全性。
...
### [116] [Strategic Feature Selection](https://arxiv.org/abs/2606.18867)
...
TL;DR: 我们提出了一种在特征选择与岭正则化共同作用下，降低算法决策系统中战略行为影响的实用方法，并通过实际医疗支付数据验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 在医疗等高风险决策场景中，算法预测器面临被操纵的数据特征，传统的重新设计模型难以实现，组织只能通过修改特征或正则化等粗级手段来应对。

Method: 对特征子集在最优岭回归正则化下的性能进行细粒度刻画，并提出联合选择特征集和正则化强度的可执行算法。

Result: 发现仅依据特征可操纵性排除特征往往不是最优策略，所提算法能在特征选择与岭正则化之间权衡，且在真实医疗支付基准案例中展示实用效能。

Conclusion: 提供了一套理论与实践相结合的框架，帮助在有限的“粗粒度”政策杠杆下削弱特征操纵带来的影响。
...
### [117] [Anomaly Detection for Sparse and Irregular Multivariate Time Series with Latent SDEs](https://arxiv.org/abs/2606.18898)
...
### [118] [REVES: REvision and VErification--Augmented Training for Test-Time Scaling](https://arxiv.org/abs/2606.18910)
*Yuanxin Liu,Ruida Zhou,Xinyan Zhao,Amr Sharaf,Hongzhou Lin,Arijit Biswas,Mohammad Ghavamzadeh,Zhaoran Wang,Mingyi Hong*

Main category: cs.LG

TL;DR: 提出的两阶段迭代修订框架通过转换中间错误为独立提示，提升LLM推理效果，减少计算成本，获得多任务SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统后训练方法仅优化一次性目标，无法匹配多步推理动态；多轮RL直接优化轨迹无法充分利用中间错误的高质量信息，需要更有效的训练机制。

Method: 采用两阶段迭代：先在线进行数据/提示增强，随后对策略优化；把成功回溯轨迹中的“near‑miss”答案拆解为修正提示和验证提示，聚焦答案转换与错误识别。

Result: 在LiveCodeBench上，使用公开测试案例作为反馈，比RL基线提升+6.5点，比标准多轮训练提升+4.0点；在circle packing、n_queens、mini_sudoku等任务上也取得SOTA或更优表现，且使用更小模型和更少采样。

Conclusion: 通过将中间步骤转化为独立的修正与验证提示，提出的两阶段迭代框架显著提升LLM推理性能，实现了更高效的数据生成与更低计算成本，并在多种任务上取得SOTA表现。
...
### [119] [Some Complexity Results for Robustness Verification for Binarized Neural Networks](https://arxiv.org/abs/2606.18918)
...
### [120] [GrapNet: A Programmable Dynamic-Architecture Neural Graph Substrate](https://arxiv.org/abs/2606.18923)
*Zirong Li*
...
### [121] [Zero-Shot Active Feature Acquisition via LLM-Elicitation](https://arxiv.org/abs/2606.18933)
...
### [122] [Be Your Own Teacher: Steering Protein Language Models via Unsupervised Reward Optimization](https://arxiv.org/abs/2606.18961)
...
### [123] [Online Reward-Punishment Learning from Fixed-Channel Perceptual Event Streams without Environment Rewards](https://arxiv.org/abs/2606.18963)
*Zirong Li*
...
### [124] [EfficientRollout: System-Aware Self-Speculative Decoding for RL Rollouts](https://arxiv.org/abs/2606.18967)
...
### [125] [A Controlled Benchmark of Quantum-Latent GAN Augmentation for Brain MRI](https://arxiv.org/abs/2606.18970)
...
### [126] [DIPHINE: Diffusion-based $Φ$-ID Neural Estimator](https://arxiv.org/abs/2606.18997)
...
### [127] [A Hybrid LSTM--Vision Transformer Architecture for Predicting HRRR Forecast Errors](https://arxiv.org/abs/2606.19026)
...
### [128] [Geometric and Stochastic Analysis of Discontinuities in Sparse Mixture-of-Experts](https://arxiv.org/abs/2606.19036)
...
### [129] [Smoothness-Based Derandomization of PAC-Bayes Bounds](https://arxiv.org/abs/2606.19105)
...
### [130] [JourneyFormer: Encoding Airbnb Guest Journey with Sequence Modeling](https://arxiv.org/abs/2606.19108)
...
### [131] [Seeing Before Reasoning: Decoupling Perception and Reasoning for Shortcut-Resilient Multimodal On-Policy Self-Distillation](https://arxiv.org/abs/2606.19120)
...
### [132] [Pareto Q-Learning with Reward Machines](https://arxiv.org/abs/2606.19134)
...
### [133] [INDEQS: Informed Neural controlled Differential EQuationS](https://arxiv.org/abs/2606.19138)
...
### [134] [ChronoSurv: A Clinical Pathway-Guided Graph Framework for Multimodal Survival Analysis](https://arxiv.org/abs/2606.19140)
...
### [135] [Complementary Attention Head Pruning for Efficient Transformers](https://arxiv.org/abs/2606.19150)
...
### [136] [The Reward Was in Your Data All Along: Correcting Flow Matching with Discriminator-Guided RL](https://arxiv.org/abs/2606.19162)
...
### [138] [Compute Efficiency and Serial Runtime Tradeoffs for Stochastic Momentum Methods](https://arxiv.org/abs/2606.19179)
...
### [139] [AGDN: Learning to Solve Traveling Salesman Problem with Anisotropic Graph Diffusion Network](https://arxiv.org/abs/2606.19185)
...
### [143] [A Human-in-the-Loop Bayesian Optimization Framework for Constraint-Aware Bioprocess Development](https://arxiv.org/abs/2606.19230)
...
### [145] [SCAN: Enhance Time Series Anomaly Detection via Multi-Scale Neighborhood-Centered Clustering](https://arxiv.org/abs/2606.19255)
...
### [146] [Detecting Hidden ML Training With Zero-Overhead Telemetry](https://arxiv.org/abs/2606.19262)
...
### [147] [Structured Inference with Large Language Gibbs](https://arxiv.org/abs/2606.19264)
...
### [148] [Risk Stratification for ICU Delirium using Pervasive Ambient Sensing Information](https://arxiv.org/abs/2606.19292)
...
### [149] [Does VLA Even Know the Basics? Measuring Commonsense and World Knowledge Retention in Vision-Language-Action Models](https://arxiv.org/abs/2606.19297)
...
### [150] [P-K-GCN: Physics-augmented Koopman-enhanced Graph Convolutional Network for Deep Spatiotemporal Super-resolution](https://arxiv.org/abs/2606.19303)
...
### [151] [Diffusion-Proof: Recipe for Formal Theorem Proving Beyond Auto-Regressive Generation](https://arxiv.org/abs/2606.19315)
...
### [152] [Explaining Attention with Program Synthesis](https://arxiv.org/abs/2606.19317)
*Amiri Hayes,Belinda Li,Jacob Andreas*

Main category: cs.LG

TL;DR: 用语言模型生成程序来逼近Transformer注意力头，效果良好，可取代部分头而不显著影响性能。


<details>
  <summary>Details</summary>
Motivation: 研究人工智能的可解释性，意在用人类可理解的符号描述替代网络的黑盒行为。

Method: 计算注意力头的注意矩阵，利用预训练语言模型生成可执行Python程序来近似这些注意模式，并对程序集合进行重新排序。

Result: 生成不足1000个程序可在TinyStories上达到75%的IOU相似度，被替代的注意头对模型困惑度仅增加16%，并保持在多项问答基准上的表现。

Conclusion: 提供了一种可扩展的流程，用可执行代码逆向工程Transformer注意力头，推动神经网络符号透明化。
...
### [153] [UBP2: Uncertainty-Balanced Preference Planning for Efficient Preference-based Reinforcement Learning](https://arxiv.org/abs/2606.19328)
...
### [154] [Deep Learning-Driven Inverse Design of Doherty Power Amplifiers Using Pixelated Combiners and Dual-State Impedance Synthesis](https://arxiv.org/abs/2606.18395)
...
### [155] [Deep-Learning-Based Pixelated Microwave Filter Design and Characterization using Electro-Optical Electric-Field Measurements](https://arxiv.org/abs/2606.18402)
...
TL;DR: 使用CNN+GA实现了像素化微波滤波器的自动化合成，实验验证其性能优异，可显著提升设计效率与创新空间。


<details>
  <summary>Details</summary>
Motivation: 传统的微波滤波器设计依赖经验和预设拓扑，迭代调整耗时且局限于有限设计空间；因此迫切需要一种能自动探索、更具创造力的设计方法。

Method: 模型首先使用卷积神经网络对像素化滤波器结构进行特征提取，然后利用遗传算法在高维搜索空间内优化结构参数，从而自动生成满足目标传输特性的设计。

Result: 实验结果显示，所合成的低通滤波器在仿真与测量之间吻合度极高；通过S-参数和空间电场测量验证了7 GHz通带与9.5 GHz以上20 dB以上衰减特性；电场模式首次展现出类似耦合传输线或短路结构的特征，为AI生成设计的出现规律提供了可视化洞见。

Conclusion: 结合CNN与遗传算法的深度学习方法成功实现了像素化微波滤波器的自动合成，并通过实验验证了设计的可靠性和性能；该技术可显著扩展滤波器设计空间并缩短开发周期。
...
### [156] [Covert Multi-Hop Communications for Heterogeneous Networks With Multiple Wardens](https://arxiv.org/abs/2606.18435)
...
### [157] [Cell-Free Integrated Sensing and Communication](https://arxiv.org/abs/2606.18488)
...
### [158] [Point-Cloud-Assistant Localized Statistical Channel Prediction by Tangent Gaussian Splatting](https://arxiv.org/abs/2606.18734)
*Ye Xue,Yiheng Wang,Xinhua Shao,Qi Yan,Shutao Zhang,Tsung-Hui Chang*

Main category: eess.SP

TL;DR: PC‑TGS通过点云几何辅助的高斯模型，实现在未测位置对无线通道角谱的高效外推，提升了大尺度无线网络规划与优化的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有 LSCM 只能在测量点附近预测 APS，无法覆盖大范围未测点，限制了在真实大规模网络中的应用。

Method: 利用稀疏点云重参数化的 3D 各向异性高斯表示环境散射体，并通过切平面投影映射至局部角域，再使用深度感知电磁 splatting 聚合其贡献，最终采用闭式 Gaussian 加权平均法进行 APS 分桶并给出误差上界。

Result: 在包含 5M 点、6,310 个 RSRP 样本的城市级 LiDAR 数据集上，PC-TGS 在 APS 与 RSRP 预测精度上优于现有基线，并在 APS 外推任务中实现了更快的推理速度。

Conclusion: PC-TGS通过结合稀疏无线测量与密集 LiDAR 形状，实现了对未测量位置 APS 的精准外推，显著提升了大规模数字孪生中的无线通道预测效率与准确性。
...
### [159] [EH-FedSAG: Variance-Reduced Federated Learning with Energy-Aware Participation in Energy-Harvesting IoT](https://arxiv.org/abs/2606.18758)
*Shahab Jahanbazi,Mateen Ashraf,Richard Demo Souza,Onel L. A. Lopez*

Main category: eess.SP

TL;DR: 在能量可靠性低且数据分布不均的网络环境下，EH‑FedSAG 相比传统 EH‑FedAvg 能显著提升模型准确率并降低训练波动。


<details>
  <summary>Details</summary>
Motivation: 在能量采集网络中，采用联邦学习（FL）面临能量到达的间歇性与随机性导致设备参与度不稳定，以及在有限能量预算下高通信成本的问题，从而降低整体训练效率。

Method: 本文在基于时隙的能量采集模型下提出了EH‑FedSAG——一种基于服务器记忆的方差削减方法。采用与传统EH‑FedAvg相同的多通道正交多址上行模型，在统一的仿真框架中比较两种算法，该框架考虑电池充电、局部计算成本和不同能量到达概率下的传输成本。

Result: 实验表明，EH‑FedSAG在服从同质与异质数据分布的条件下，均能实现更高的测试准确率，并且训练方差显著下降。特别是在能量稀缺和数据非独立同分布（non‑IID）情况下，EH‑FedSAG的优势更为明显。

Conclusion: EH‑FedSAG在能量约束和通信成本受限的能源采集网络中，通过方差削减显著提升了训练准确率和稳定性，是改进FL性能的有效方案。
...
### [160] [Rotatable Antenna-Enhanced Secure Integrated Sensing and Communications Under Imperfect CSI](https://arxiv.org/abs/2606.18766)
...
### [161] [Controlled Out-of-Band Device-to-Device Communication in Cellular Networks Using a Backup Channel in Television White Space](https://arxiv.org/abs/2606.18827)
*Saifur Rahman,Syed Luqman Shah,Salim Nasar Faraj Mursal,Ziaul Haq Abbas,Muhammad Usman,Muhammad Irfan,Fazal Muhammad*

Main category: eess.SP

TL;DR: 利用电视白名单作为备份信道，降低阻塞/延迟，增强蜂窝网络容量


<details>
  <summary>Details</summary>
Motivation: 解决蜂窝网络中的频谱稀缺问题

Method: 为同一宏基站覆盖范围内的用户提供备份信道（BuC），在电视空白频段工作，通过认知无线电能量检测进行感知，并在常规通道占用时建立设备间的外道链路，减轻核心网络负载

Result: 仿真表明阻塞概率和通话延迟概率显著降低

Conclusion: 备份信道方案可提高宏基站容量并有效缓解频谱资源短缺
...
### [162] [Spaceborne SAR Change Detection and Coherence Analysis for Maritime Port Monitoring](https://arxiv.org/abs/2606.18917)
*Necati Kagan Erkek,Kudret Esmer*

Main category: eess.SP

TL;DR: 本文通过对天津港SAR幅值与相干数据的全流程处理与多时相比较，证明了SAR在多变港口环境中高精度、连续基础设施监测的价值。


<details>
  <summary>Details</summary>
Motivation: 利用空间干涉合成孔径雷达（SAR）实现不受光照和大气影响的海事基础设施监视，满足对天津港复杂港口设施的高频、精细检测需求。

Method: 系统采用幅值可视化、放射计量归一化、视角解释、分辨率评估、斑点抑制、幅值差异变异图、GeoTIFF导出及相干映射；使用直方图引导的显示限、两维频谱分析估计分辨率；多时相主/伴图像通过滤波幅值差与多窗口相干率比较，实现变化检测。

Result: 研究获得约0.42 m的范围分辨率和0.19°的方位角分辨率；通过幅值差与相干率对比，成功识别港口船舶、储罐、堆场、码头结构及水陆交界处的结构与表面变化。

Conclusion: 本文表明，在多样化港口环境下，SAR幅值与相干产品能够有效监测结构与表面状况的变化，提供可靠的非照明、非天气限制的海事基础设施监测手段。
...
### [163] [Channel Charting With Physical Channel Fingerprints For Massive MIMO-OFDM Channel Acquisition](https://arxiv.org/abs/2606.19010)
...
### [164] [Structure Over Nonlinearity: Explicit Interaction Architectures for Dynamical Learning](https://arxiv.org/abs/2606.19101)
*Augusto Sarti*
...
### [165] [Decentralized Power Control for Over-the-Air Computation with Phase Noise](https://arxiv.org/abs/2606.19102)
...
### [166] [Pushing the Limits: Unlocking the Potential of Faster-than-Nyquist Signaling](https://arxiv.org/abs/2606.19226)
*Zichao Zhang,Melda Yuksel,Shuangyang Li,Gokhan M. Guvensen,Halim Yanikomeroglu*

Main category: eess.SP

TL;DR: FTN能通过压缩符号间隔提升频谱效率，实际应用需解决功放、峰值功率、编码及接收机复杂度等挑战，适用于低时延、感知与卫星通信等场景。


<details>
  <summary>Details</summary>
Motivation: 在频谱资源日益紧张的背景下，传统符号间隔受限无法满足高速通信需求，FTN通过故意打破符号间距规则来拥塞更多数据。

Method: 通过对不同加速因子与信噪比定义下的性能进行理论与数值分析，结合功放非线性、峰均功率、实际编码方案等实际问题，评估FTN在真实环境中的可行性。

Result: 结果显示FTN在适当的加速因子和SNR标准下可提升容量，但其优势受制于功放非线性、PAPR高以及解调复杂度，需要针对性技术解决。

Conclusion: FTN信号在充分考虑功放限制、峰均功率、编码策略及接收机复杂度后，能够在有限频谱内显著提升容量，为低时延链接、集成感知及卫星通信等场景提供了具有吸引力的发展方向。
...
### [167] [Learning-Based Decision Making for Combustion Phasing Control in Multi-Fuel CI Engines with Latent Fuel Reactivity Estimation](https://arxiv.org/abs/2606.18393)
*Rajasree Sarkar,Aditya Satish Patil,Arunava Banerjee,Ihsan Berk Altiner,Zongxuan Sun,Kenneth Kim,Chol-Bum Mike Keown*

Main category: eess.SY

TL;DR: 研究多燃料压燃发动机CA50调节，提出基于GRU的RL框架，获准在未见燃料活性曲线下稳定控制，误差低于0.25° CA，显示对动态燃料活性的自适应逼近优于传统估计-控制链路。


<details>
  <summary>Details</summary>
Motivation: 多燃料压燃发动机提供燃料灵活性，但燃料活性（Cetane数）随时间变化且不确定，导致燃烧相位控制不稳定。

Method: 将CA50调节视为部分可观测的序列决策问题，比较LinUCB、历史增强上下文银弹、仅观测DDPG、循环DDPG以及提出的GRU引导RL框架。使用实验多燃料发动机数据训练的高斯过程代理构建可控可重复的评估环境。

Result: 单调或固定历史的带迴方法在CN变化下表现下降；仅观测RL产生潜在状态别名；通用递归对快速CN演变不足。GRU框架能从燃烧历史学习紧凑的燃料活性表示，并在控制器中使用该估计而非oracle CN。未见CN轨迹下，平均绝对跟踪误差<0.25° CA，SOI和点火功率平滑且物理一致。

Conclusion: 燃烧控制在潜在、持续演化的燃料动力学下，需要将燃料活性推断与控制策略学习对齐；本框架通过使用部署阶段可获得的估计状态实现了对燃料活性的充分利用。
...
### [168] [Constellation-Level Power Allocation for LEO Space-Based Solar Power](https://arxiv.org/abs/2606.18416)
*Mustafa Alhassan,Amjad Iqbal,Peng Hu*

Main category: eess.SY

TL;DR: 本文在450 km轨道上通过24小时仿真验证Walker 4×5 LEO星座能向8站传输幅度1.986MW峰值、40–75kW平均功率，功率密度低于安全阈值。建议20颗星座可实现约50–100kW/站，天线尺寸依据5 W/m² PD设计


<details>
  <summary>Details</summary>
Motivation: 研究低地球轨道（LEO）卫星星座在空间能源（SBSP）中的潜力，填补现有研究中缺乏模拟分析的空白

Method: 构建Walker 4×5 LEO SBSP系统模型，开展全天系统级仿真，考虑轨道传播、日蚀周期、卫星功率链、Goubau–Brown 波束耦合、ITU‑R P.618 大气衰减及电池动力学；采用贪婪分配策略向8个地面站进行2.45 GHz微波功率传输

Result: 峰值直流功率1.986MW，单站平均供电40–75kW；两站因优先级低未得到服务；脉冲功率密度3.35–5.72 W/m²，低于ICNIRP安全标准。20颗卫星Walker LEO方案可实现50–100kW每站供电，射频天线尺寸应依据约5 W/m²的实际PD而非传统100 W/m²规格

Conclusion: 低地轨SBSP可提供稳定清洁能量，安全符合辐射限制；系统设计需关注轨道竞争与功率密度，采用精细化分配可提升覆盖与效率
...
### [169] [Stability of Slow-Fast Nonlinear Dynamics: Non-Periodic Case](https://arxiv.org/abs/2606.18504)
*G. Q. Bao Tran,Daniel Liberzon,Hyungbo Shim*

Main category: eess.SY

TL;DR: 用一般平均法给出非周期快变系统的半全局指数稳定性条件，适用于换轨系统等


<details>
  <summary>Details</summary>
Motivation: 研究非周期快变系统的半全局指数稳定性，适用于换轨非周期系统和准周期变化

Method: 采用一般平均法构造平均系统，假设在冻结慢变时平均系统存在全局指数稳定的时间不变平衡，并考虑慢变可断续但总变差有界

Result: 给出充分条件保证原系统半全局指数稳定，举例验证在慢快非周期换轨的非线性系统上

Conclusion: 证明非周期快变在满足平均系统稳定性及慢变有界变差条件时，可实现系统的半全局指数稳定，拓展了对换轨系统的稳定性分析范围
...
### [170] [Wind-Resilient Trajectory Optimization for UAV-BS Networks: TD3 for Continuous Service Availability](https://arxiv.org/abs/2606.18556)
...
### [171] [LQR based stabilization of an 1D heat equation with advection and memory effects](https://arxiv.org/abs/2606.18761)
*Bhargav Pavan Kumar Sistla,Vivek Natarajan*

Main category: eess.SY

TL;DR: 论文提出并验证了一种通过有限维LQR近似实现一维光传热带记忆和对流的指数稳态控制的方法，证明在满足特定正交性条件下可实现所需的衰减速率，并通过数值实验证实可行性。


<details>
  <summary>Details</summary>
Motivation: 在有记忆项和对流的热传输问题上，传统控制方法难以保证所需的指数衰减速率；本研究尝试以有限维LQR近似来实现严格的指数稳态控制，从而填补控制理论与实际热传输系统的空缺。

Method: 1) 建立热传递的偏微分方程并解析其谱结构；2) 采用闭环反馈法，构造算子A和控制算子B；3) 对A进行谱闭包分析，说明谱为其特征值的闭包；4) 对有限维逼近(A_n,B_n)求解代数Riccati方程得到增益K_n；5) 证明足够大n时A_n+B_nK_n的谱满足Reλ<-ω，从而实现ω-稳态。

Result: 分析得出算子A的特征值趋向于有限负积累点，决定了最大可实现的衰减率；在该积累点以下，若控制算子满足非正交性可实现稳定；通过有限维逼近，可得到收敛至原系统的增益，数值实验验证了方法有效性。

Conclusion: 作者提供了一种一维热传递模型的指数记忆和对流边界条件下的ω-稳态控制方案，并证明在足够大的截断维数下，利用有限维LQR近似即可得到实现指数衰减的闭环系统，且该方案在控制器满足非正交性条件时可行。
...
### [172] [PowerAgentBench-SS: A Benchmark for Agentic AI in Power System Steady-State Studies](https://arxiv.org/abs/2606.18789)
*Costas Mylonas,Magda Foti,Andrea Pomarico,Matheus Duarte,Qian Zhang,Emmanouel Varvarigos*

Main category: eess.SY

TL;DR: 介绍了一个面向电力系统稳态任务的LLM代理基准，详细说明了框架设计、评价指标和实验结果，强调了完整工作流评估的重要性


<details>
  <summary>Details</summary>
Motivation: 传统的电力系统基准主要评估数值求解器、预测模型或顺序控制器，但并未直接考察大语言模型（LLM）代理是否能执行完整的工程工作流，从检视电网案例到调用仿真工具、筛选故障并给出可接受的缓解措施，并验证结果与生成可审计证据；此研究旨在填补这一空白

Method: 提出PowerAgentBench‑SS稳态基准框架，公开案例数据、操作约束、工具API和验证预算，并通过隐藏评估器重算物理有效性和打分；定义代理接口、工具合同、证据日志和风险敏感指标，并在IEEE 39‑bus N‑2 故障搜索实验中实现该协议，结合脚本基线、LLM JSON命令适配器、三个本地Ollama LLM代理及一个OpenAI API代理进行可复现测试

Result: 实验表明，仅靠求解器或答案评估不足以区分代理；评分体系能体现顶级故障发现、验证预算使用、显式提交、类型转换、重复验证、基于证据的报告和缓解行为等差异，对代理性能提供更细粒度评价

Conclusion: PowerAgentBench‑SS为评估工具使用型LLM代理在电力系统运维与规划中的完整工作流提供了一套系统化、可复现且具风险敏感性的基准框架，可促使LLM代理在实际工程场景中获得更可靠和透明的表现
...
### [173] [A Theory-Guided Advanced Regulatory Control Synthesis for Cooling-Limited Exothermic Semi-Batch Reactors](https://arxiv.org/abs/2606.18799)
...
### [174] [Bridging Data-Driven and Model-Based Methods: A Learn-to-Optimize Architecture for Distributed Optimal Power Flow](https://arxiv.org/abs/2606.18822)
...
### [175] [From Tokens to Energy Flexibility: Quantization-Enabled Demand Response for Data Centers with LLM Inference Workloads](https://arxiv.org/abs/2606.18851)
...
### [176] [A Finite-Gain Stability Approach to NMPC Design: the Extended Version](https://arxiv.org/abs/2606.18863)
*Carlo Novara,Mattia Boggio,Lorenzo Calogero,Michele Pagone*

Main category: eess.SY

TL;DR: 利用有限增益稳健性设计 NMPC，克服未知扰动下传统方法的局限，仿真验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的拉普拉斯稳定性分析在存在未知但有界扰动时难以直接应用，故需寻找更具普适性的设计方法。

Method: 在闭环系统满足有限增益稳定性条件的框架下，系统化地设计 NMPC 参数，以保证闭环的 FGS 性质及时间变参考轨迹的追踪性能。

Result: 仿真演示说明所提框架在自动驾驶车辆的横纵向控制中能够实现良好的追踪性能。

Conclusion: 通过基于有限增益稳定性的 NMPC 设计，实现了在未知但有界扰动下的闭环系统的稳健跟踪控制，并在自动驾驶车辆示例中验证了方法的有效性。
...
### [177] [Model-Free Reinforcement Learning Control for Resilient Cyber-Physical Systems](https://arxiv.org/abs/2606.19069)
...
TL;DR: 比较四种 RL 奖励在非线性系统下对抗网络攻击的表现，Lyapunov 奖励最稳健；指数奖励次之；RL‑PID 快速训练；PPO 在稳定性上优于 DDPG。


<details>
  <summary>Details</summary>
Motivation: 提供一种评估和优化鲁棒强化学习控制器在面对网络攻击（误报注入和拒绝服务）时表现的方法，寻找奖励设计对系统安全和性能的影响。

Method: 对四种RL奖励（Lyapunov、指数、进阶、线性）在非线性系统模型中进行仿真，并对比RL‑MPC、RL‑PID控制器使用PPO与DDPG训练。通过评估跟踪误差、成本、鲁棒性等指标量化性能。

Result: 实验结果表明 Lyapunov 奖励产生最低跟踪误差和最高鲁棒性；指数奖励在适度训练下亦良好；进阶与线性奖励收敛快但鲁棒性差。RL‑MPC在稳态时鲁棒性高但训练时间长；RL‑PID训练速度快。PPO 优于 DDPG，在 KPI 变异方面降低显著。

Conclusion: 在面对网络攻击时，Lyapunov奖励工​​具能提供最佳的鲁棒性且跟踪误差低；指数模式在训练条件适中时也表现出良好的折中；进阶和线性奖励收敛快但鲁棒性不足。RL‑PID控制器训练更快，RL‑MPC稳态鲁棒性强但训练耗时更长；PPO在KPI方差上显著优于DDPG。
...
### [178] [Byzantine-Resilient Federated Multi-Agent Optimization Framework for Cyber-Secure Interconnected Microgrids](https://arxiv.org/abs/2606.19080)
*Ali Peivand,Seyyed Mostafa Nosratabadi*

Main category: eess.SY

TL;DR: 提出一套BFT联邦强化学习框架，能在不泄露关键参数的情况下，利用多层移动目标防御和自适应隔离，成功抵御微网尺度的隐蔽数据注入攻击。


<details>
  <summary>Details</summary>
Motivation: 数字化配电网络的快速发展使微网群组易受到隐蔽误数据注入攻击，攻击可绕过传统错误检测器并通过跨线耦合与共享学习渠道传播，亟需一种既不泄露电网机密信息又能抵御拜占庭故障与恶意模型操纵的安全联邦学习方案。

Method: 采用拜占庭容错联邦多智能体近端策略优化（Federated MAPPO）进行学习，策略分为全局共享编码器和本地私有动作头；结合三层移动目标防御与自适应隔离策略，利用双阶段拜占庭容错聚合规则（修剪均值滤波+奖励加权更新），并引入基于F1-score与FPR的检测质量惩罚。

Result: 在IEEE 30/118节点测试系统的四个互联微网上仿真验证，结果显示能够稳健地抑制协调误数据注入攻击、通过自适应隔离控制级联失效，并在不影响成本调度的前提下，保障分布式学习通道安全。

Conclusion: 本工作通过提出BR-FedMAPPO框架，实现了在分布式马达学习环境下对微网系统的欺骗式隐蔽数据注入攻击的有效防御，既维持了功率调度的成本效益，又保障了网络的安全与鲁棒性。
...
### [179] [Integrated physics-based modeling reveals a thermodynamic gap in small modular reactor load following](https://arxiv.org/abs/2606.19296)
*Ali Mahboub Rad,Roshni Anna Jacob,Bikash Poudel,Mayir Mamtimin,Jie Zhang*

Main category: eess.SY

TL;DR: 开发耦合SMR动态模型，发现只有全三具点并行控制才能稳定负荷跟随；耦合显著影响瞬态与蒸汽流量预测，提示需采用热力耦合模型以提升评估精准度。


<details>
  <summary>Details</summary>
Motivation: 现有SMR动力学研究常忽视主次回路热力耦合，导致对负荷跟随能力的评估不准确。

Method: 构建混合动态框架，将NuScale整体压水阀式核反应堆（主回路及自旋边界螺旋螺纹一次抽汽机组）与物理力学式二次蒸汽循环（阀门、汽轮机、冷凝器、补水泵）耦合，保证质量和能量守恒，并在域界面保持物理一致的流相互作用。

Result: 对5\%负荷拒绝采用五种控制策略（包括分散式三级控制）进行仿真。结果显示单独或部分控制不足，必须同时调整阀门、补水泵与控制棒才能稳定蒸汽压力、限制主回路热冲击并维持蒸汽发生器安全裕度。相较传统线性蒸汽循环模型，耦合框架能捕获动态背压与可变汽轮机焓降，改变了瞬态预测与所需蒸汽流量。

Conclusion: 需采用热力耦合、物理基础的蒸汽循环模型，才能更准确评估SMR在真实负荷跟随条件下的运行灵活性、效率与安全裕度。
...
### [180] [A Lyapunov-Based Perspective on Absolute Stability](https://arxiv.org/abs/2606.19311)
*Tessina H. Scholl*
...
### [181] [A skew polynomial framework for constructing division algebras and linear maximum rank distance codes](https://arxiv.org/abs/2606.18371)
*Susanne Pumpluen*
...
### [182] [Holographic Integrated Sensing and Communication With Limited Radiation Amplitudes: How Many Quantization Bits Are Enough?](https://arxiv.org/abs/2606.18456)
*Shuhao Zeng*

Main category: cs.IT

TL;DR: 研究RIS-ISAC中有限振幅量化对性能的影响，推出量化位数上限并验证。


<details>
  <summary>Details</summary>
Motivation: 现实中RIS元件难以实现连续振幅控制，需研究有限量化对ISAC系统性能的影响以指导设计。

Method: 利用闭式下界推导通信速率与感知 SINR 的上界，并基于此确定量化精度上限；随后分析通信-感知性能权衡对量化位数的影响。

Result: 得到通信速率与感知 SINR 的下界公式；给出最小量化位数的上限；数值仿真验证理论推导。

Conclusion: 本研究表明，有限辐射振幅量化位数对RIS-ISAC 性能影响显著，并给出了上限量化位数的闭式下界。
...
### [183] [Covert Blockwise Coding with Sequential Detection over Thermal-Loss Bosonic Channels](https://arxiv.org/abs/2606.18666)
...
### [184] [Environment-Aware Resource Allocation for Pinching-Antenna-Assisted EDMA-NOMA Systems](https://arxiv.org/abs/2606.18899)
*Yaxuan Luo*
...
### [185] [Structure of kissing arrangements in ${\mathbb R}^{12}$ and a place for the $841$st sphere](https://arxiv.org/abs/2606.18984)
*Rustem Takhanov,Zhenisbek Assylbekov,Stanislav Yun*

Main category: cs.IT

TL;DR: 通过分析桥向量固定的亲吻阵列，发现子块具有可变自由度，进而构造无限多种840点阵列，并利用该结构首次得到841点的亲吻配置


<details>
  <summary>Details</summary>
Motivation: 研究在$\mathbb R^{12}$中大小为840的亲吻阵列，探究其结构与灵活性

Method: 固定桥向量，研究60点子块的可变性，构造48维系统并使用其作为Riesz能量优化的初始化方案

Result: 证明每个60点子块具有可变性，得到无穷多非等距的大小840亲吻阵列，随后利用该结构构造大小841亲吻阵列

Conclusion: 揭示了极大配置的局部结构自由度，为能量优化和亲吻数研究提供新方法
...
### [186] [The Simplicity Paradox: Why Evolution Does Not Produce Universally Complex Agents](https://arxiv.org/abs/2606.19136)
*Teddy Lazebnik*

Main category: cs.IT

TL;DR: 研究提出认知经济学框架，说明复杂环境下多样化与专业化可降低决策成本并保留知识价值，解释社会为何常保简化。


<details>
  <summary>Details</summary>
Motivation: 探究为何人类在信息经济显性价值时仍未趋向全复杂，解决知识动物性与简化偏好之间的进化难题。

Method: 对比全复杂代理社会与包含简化代理及专业决策中心的社会，定量比较信息获取成本与收益，并通过理论模型分析分工优势。

Result: 发现当分布式简化触发的成本节约超过个体自主与委托失效带来的价值损失时，存在专门决策者的社会结构会占优势，且其私利可高于普遍复杂化。

Conclusion: 简单化并非适应失败，而是可扩缩社会组织的先决条件。
...
### [187] [Ray Antenna Array Enhanced Low-Altitude ISAC: Performance Analysis and Beamforming Design](https://arxiv.org/abs/2606.19146)
...
### [188] [Reconstruction Limits for Repeated Differentially Private Aggregates: A Cramer-Rao Perspective on Query Geometry](https://arxiv.org/abs/2606.19275)
*Chenyue Zhang,Andrew Campbell,Anna Scaglione,Sean Peisert*

Main category: cs.IT

TL;DR: 本文指出差分隐私多次发布的局部重构风险取决于发布几何而非单纯的发布长度，分析了固定背景、有序多重和多项式矩等场景下的风险阶数，并揭示了隐私预算模型对风险的影响。


<details>
  <summary>Details</summary>
Motivation: 评估在多次发布下，隐私预算和发布几何如何共同影响局部重构风险。

Method: 通过高斯校准的重复统计查询，分析利基化Fisher几何和逆参数化的特征线性最优估计；结合基本求和、zCDP/RDP计数模型和霍尔德矩为实验框架。

Result: 针对固定背景的标记重构、盲注对称性和多项式矩，证明在不同计数模型下风险的阶数变化，揭示了有效发布次数与噪声增长的平衡点。

Conclusion: 差分隐私多次发布的本地重构风险主要由发布的几何结构决定，而非单纯的长度或累计隐私计数。
...
### [189] [Secret key-distribution over networks with node-based adversarial errors](https://arxiv.org/abs/2606.19305)
